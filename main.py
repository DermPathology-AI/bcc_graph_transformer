...
)  # gamma=0.3  # 30,90,130 # 20,90,130 -> 150
    ...
    if train:
        for i_batch, sample_batched in enumerate(dataloader_train):
            ...
            if (i_batch + 1) % args.log_interval_local == 0:
                print(
                    "[%d/%d] train loss: %.3f; agg acc: %.3f"
                    % (
                        total,
                        total_train_num,
                        np.mean(np.array(train_loss.tolist())) / total,
                        trainer.get_scores(),
                    )
                )
                # trainer.plot_cm()
    ...
    if epoch % 1 == 0:
        with torch.no_grad():
            ...
            for i_batch, sample_batched in enumerate(dataloader_val):
                ...
                preds, labels, loss, p = evaluator.eval_test(
                    sample_batched, model, graphcam
                )
                ...
                if (i_batch + 1) % args.log_interval_local == 0:
                    print(
                        "[%d/%d] val agg acc: %.3f"
                        % (total, total_val_num, evaluator.get_scores())
                    )
                    # evaluator.plot_cm()
                    # accuracy, auc_value, precision, recall, fscore = five_scores(labels, preds)
            ...
            print(
                "[%d/%d] val agg acc: %.3f"
                % (total_val_num, total_val_num, evaluator.get_scores())
            )
            evaluator.plot_cm()
            ...
            log = (
                log
                + "epoch [{}/{}] ------ acc: train = {:.4f}, val = {:.4f}".format(
            ...
            log_clean = (
                log_clean
                + "{} {:.4f} {:.4f}".format(
                    epoch + 1, trainer.get_scores(), evaluator.get_scores()
                )
                + "\n"
            )
            ...
            writer.add_scalars(
                "accuracy",
                {"train acc": trainer.get_scores(), "val acc": evaluator.get_scores()},
                epoch + 1,
            )
