...
def main(args):
    ...
    patch_info = open(os.path.join(args.path_graph, file_name + '_files', 'c_idx.txt'), 'r') # Filmon added_files
    ...
    mag_scale = 3.5
    w_s, h_s = float(512/20)*mag_scale, float(512/20)*mag_scale # Filmon: Depending on the magnification size multiply by 1,2,or 4 
    print(w_s, h_s)
   ...
   # GraphCAM
    ...
    cv2.imwrite('graphcam_vis/cam_all.png'.format(file_name, site), np.rot90(vis_merge,axes=(1, 0)))
    cv2.imwrite('graphcam_vis/{}_{}_cam_all_{}.png'.format(file_name, site, str(round(max(p),5))), np.rot90(vis_merge,axes=(1, 0)))
    cv2.imwrite('graphcam_vis/{}_{}_ori.png'.format(file_name, site), np.rot90(output_img, axes=(1, 0)))
    
    #for i in range(1, len(vis_all)):
    cv2.imwrite(f'graphcam_vis/{file_name}_{site}_cam_{p_pred}.png', np.rot90(vis, axes=(1, 0)))
