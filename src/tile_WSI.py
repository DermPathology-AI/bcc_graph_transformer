...
class TileWorker(Process):
    ...
    def __init__(
        self, queue, slidepath, tile_size, overlap, limit_bounds, quality, _Bkg, _ROIpc
    ):
        ...
        while True:
            ...
            if True:
                try:
                        ...
                        if PercentMasked >= (self._ROIpc / 100.0):
                            if Normalize != "":
                                ...
                                tile = Image.fromarray(
                                    self.normalize_tile(tile, Normalize).astype(
                                        "uint8"
                                    ),
                                    "RGB",
                                )
                            ...
                            if bool(SaveMasks) == True:
                                ...
                                TileMaskO = numpy.array(
                                    Image.fromarray(TileMaskO).resize(
                    ...
                    # print(level, address)
                    print(
                        "image %s failed at dz.get_tile for level %f"
...
class DeepZoomImageTiler(object):
    ...
    def _write_tiles(self):
        ...
        try:
            ...
            if (
                ("jpg" in self._ImgExtension)
                | ("dcm" in self._ImgExtension)
                | ("tif" in self._ImgExtension)
            ):
        ...
        if True:
            ...
            if (
                (self._xmlfile != "")
                & (self._ImgExtension != "jpg")
                & (self._ImgExtension != "dcm")
            ):
                # print("read xml file...")
                ...
                if xml_valid == False:
                    print(
                        "Error: xml %s file cannot be read properly - please check format"
                        % xmldir
                    )
                    return
                ...
                for row in range(rows):
                    for col in range(cols):
                        ...
                        if InsertBaseName:
                            ...
                            tilename_bw = os.path.join(
                                tiledir, "%d_%d_mask.%s" % (col, row, self._format)
                            )
                        if xml_valid:
                            ...
                            Dlocation, Dlevel, Dsize = self._dz.get_tile_coordinates(
                                level, (col, row)
                            )
                            Ddimension = tuple(
                                [
                                    pow(2, (self._dz.level_count - 1 - level)) * x
                                    for x in self._dz.get_tile_dimensions(
                                        level, (col, row)
                                    )
                                ]
                            )
                            startIndY_current_level_conv = int(
                                (Dlocation[1]) / Img_Fact
                            )
                            endIndY_current_level_conv = int(
                                (Dlocation[1] + Ddimension[1]) / Img_Fact
                            )
                            startIndX_current_level_conv = int(
                                (Dlocation[0]) / Img_Fact
                            )
                            endIndX_current_level_conv = int(
                                (Dlocation[0] + Ddimension[0]) / Img_Fact
                            )
    ...
    def xml_read(self, xmldir, Attribute_Name, Fieldxml):
        ...
        NewFact = max(ImgMaxSizeX_orig, ImgMaxSizeY_orig) / min(
            max(ImgMaxSizeX_orig, ImgMaxSizeY_orig), 15000.0
        )
        # Img_Fact =
        ...
        img = Image.new(
            "L", (int(ImgMaxSizeX_orig / NewFact), int(ImgMaxSizeY_orig / NewFact)), 0
        )
...
class DeepZoomStaticTiler(object):
    ...
    def _write_html(self):
        ...
        associated_urls = dict(
            (n, self._url_for(n)) for n in self._slide.associated_images
        )
        try:
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-x", "--xmlfile", metavar="NAME", dest="xmlfile", help="xml file if needed"
    )
    parser.add_option(
        "-F",
        "--Fieldxml",
    ...
    for imgNb in range(len(files)):
        ...
        if "dcm" in ImgExtension:
            ...
            image[..., 0] = (
                (im1[:, :].astype(float) - minVal) / (maxVal - minVal) * 255.0
            ).astype(int)
            image[..., 1] = (
                (im1[:, :].astype(float) - minVal) / (maxVal - minVal) * 255.0
            ).astype(int)
                ...
                if (opts.mask_type == 1) or (opts.oLabelref != ""):
                    ...
                    for oLabel in xml_labels:
                        ...
                        if (opts.oLabelref in oLabel) or (opts.oLabelref == ""):
                            ...
                            if opts.mask_type == 0:
                                ...
                                if not os.path.exists(
                                    os.path.join(opts.basename, oLabel + "_inv")
                                ):
                                    os.makedirs(
                                        os.path.join(opts.basename, oLabel + "_inv")
                                    )
                                ...
                                if not os.path.exists(
                                    os.path.join(opts.basename, oLabel)
                                ):
                        ...
                        if Nbr_ROIs_ForNegLabel == 0:
                            print(
                                "label %s is not in that image; invert everything"
                                % (opts.oLabelref)
                            )
