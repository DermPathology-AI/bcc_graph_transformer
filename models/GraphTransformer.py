...
class Classifier(nn.Module):
    def __init__(self, n_class):
        ...
        self.embed_dim = 128  # try with 64
        self.num_layers = 3
        self.node_cluster_num = 120  # try 100
    ...
    def forward(
        self, node_feat, labels, adj, mask, is_print=False, graphcam_flag=False
    ):
        ...
        if graphcam_flag:
            ...
            if path.exists("graphcam/att_1.pt"):
                all_cams = glob.glob("graphcam/*")
                for _cam in all_cams:
                    os.remove(_cam)
        ...
        if graphcam_flag:
            ...
            for index_ in range(num_classes):
