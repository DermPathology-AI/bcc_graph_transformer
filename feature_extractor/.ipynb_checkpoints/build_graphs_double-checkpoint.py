...
def compute_feats(args, bags_list, i_classifier, save_path=None, whole_slide_path=None):
    ...
    for i in range(0, num_bags):
        ...
        if os.path.isdir(os.path.join(save_path, 'simclr_files_double', file_name_files)) or len(csv_file_path) < 1:
            print('already exists')
            continue
        #with torch.no_grad():
        #    for iteration, batch in enumerate(dataloader):
        #        continue                                                                              # Filmon added continue 
        ...
        os.makedirs(os.path.join(save_path, 'simclr_files_double', file_name_files), exist_ok=True)
        
        txt_file = open(os.path.join(save_path, 'simclr_files_double', file_name_files, 'c_idx.txt'), "w+")
        save_coords(txt_file, csv_file_path)
        # save node features

        #output = torch.stack(feats_list, dim=0).cuda()
        
        features_double = pd.read_csv(f'/workspace/data/cv_methods/dsmil-wsi/datasets/bcc/{class_name}/{file_name}.csv')
        
        output = torch.tensor(np.array(features_double)).cuda()
        #save_path = '/workspace/data/cv_methods/tmi2022/graphs/'
  
        torch.save(output, os.path.join(save_path, 'simclr_files_double', file_name_files, 'features.pt'))
    
        # save adjacent matrix
        adj_s = adj_matrix(csv_file_path, output)
        torch.save(adj_s, os.path.join(save_path, 'simclr_files_double', file_name_files, 'adj_s.pt'))
        ...
        features_double = torch.load(f'/workspace/data/cv_methods/tmi2022/graphs/simclr_files_double/{file_name}_files/features.pt', map_location=lambda storage, loc: storage)
...
def main():
    ...
    bags_list = glob.glob('/workspace/data/cv_methods/dsmil-wsi/WSI/bcc/pyramid/*/*/')
    print(bags_list)
