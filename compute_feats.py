...
def bag_dataset(args, csv_file_path):
    transformed_dataset = BagDataset(
        csv_file=csv_file_path, transform=Compose([ToTensor()])
    )
...
def compute_feats(
    args, bags_list, i_classifier, save_path=None, magnification="single"
):
    i_classifier.eval()
    ...
    for i in range(0, num_bags):
        ...
        if len(feats_list) == 0:
            ...
            if magnification == "low":
                os.makedirs(
                    os.path.join(
                        save_path + "_low", bags_list[i].split(os.path.sep)[-2]
                    ),
                    exist_ok=True,
                )
                df.to_csv(
                ...
                os.makedirs(
                    os.path.join(
                        save_path + "_high", bags_list[i].split(os.path.sep)[-2]
                    ),
                    exist_ok=True,
                )
                df.to_csv(
...
def compute_tree_feats(
    args, bags_list, embedder_low, embedder_high, save_path=None, fusion="fusion"
):
    _weight_low = 0.25
                    ...
                    for high_patch in high_patches:
                        ...
                        if fusion == "cat":
                            feats = np.concatenate(
                                (
                                    feats.cpu().numpy(),
                                    _weight_low * feats_list[idx][None, :],
                                ),
                                axis=-1,
                            )
                            feat_high_patches.extend(feats)
...
def compute_tree_feats_separate(
    args, bags_list, embedder_low, embedder_high, save_path=None, fusion="fusion"
):
...
def main():
    ...
    parser.add_argument(
        "--num_workers", default=64, type=int, help="Number of threads for datalodaer"
    )
    parser.add_argument(
        "--gpu_index", type=int, nargs="+", default=(0, 1, 2, 3), help="GPU ID(s) [0]"
    )
    parser.add_argument(
        "--backbone", default="resnet18", type=str, help="Embedder backbone [resnet18]"
    )
    parser.add_argument(
        "--norm_layer",
        default="instance",
    ...
    parser.add_argument(
        "--dataset", default="TCGA-lung-single", type=str, help="Dataset folder name"
    )
    parser.add_argument("--holdout_set", default=False, help="Used to create datasets")
    parser.add_argument(
        "--separate_feat", default=False, help="Generated features separately"
    )
    ...
    if (
        ...
        and args.weights_high != None
        and args.weights_low != None
    ):
        ...
        if args.weights == "ImageNet":
            ...
            os.makedirs(os.path.join("embedder", args.dataset), exist_ok=True)
            torch.save(
                new_state_dict, os.path.join("embedder", args.dataset, "embedder.pth")
            )
            print("Use pretrained features.")
    ...
    if args.magnification == "tree":
        if args.separate_feat:
            ...
            compute_tree_feats(
                args, bags_list, i_classifier_l, i_classifier_h, feats_path, "fusion"
            )
    ...
    if args.holdout_set:
        n_classes = glob.glob(
            os.path.join("datasets_test_set", args.dataset, "*" + os.path.sep)
        )
    else:
    ...
    for i, item in enumerate(n_classes):
        ...
        if args.holdout_set:
            ...
            bag_df.to_csv(
                os.path.join(
                    "datasets", args.dataset, item.split(os.path.sep)[2] + ".csv"
                ),
                index=False,
            )
    ...
    if args.holdout_set:
        bags_path.to_csv(
            os.path.join("datasets_test_set", args.dataset, args.dataset + ".csv"),
            index=False,
        )
    else:
        bags_path.to_csv(
