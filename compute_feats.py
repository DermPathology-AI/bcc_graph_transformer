    ...
    for i in range(0, num_bags):
        ...
        if len(feats_list) == 0:
            ...
            if magnification=='low':
                os.makedirs(os.path.join(save_path+'_low', bags_list[i].split(os.path.sep)[-2]), exist_ok=True)
                df.to_csv(os.path.join(save_path+'_low', bags_list[i].split(os.path.sep)[-2], bags_list[i].split(os.path.sep)[-1]+'.csv'), index=False, float_format='%.4f')
            elif magnification=='high':
                os.makedirs(os.path.join(save_path+'_high', bags_list[i].split(os.path.sep)[-2]), exist_ok=True)
                df.to_csv(os.path.join(save_path+'_high', bags_list[i].split(os.path.sep)[-2], bags_list[i].split(os.path.sep)[-1]+'.csv'), index=False, float_format='%.4f')
            else:
        ...
        for i in range(0, num_bags): 
            ...
            for idx, low_patch in enumerate(low_patches):
                ...
                if len(high_patches) == 0:
                    feats = _weight_low*feats_list[idx]
                    feat_high_patches.extend(feats)
                    ...
                    for high_patch in high_patches:
                        ...
                        if fusion == 'fusion':
                            feats = feats.cpu().numpy() + _weight_low*feats_list[idx]
                            feat_high_patches.extend(feats)

                        if fusion == 'cat':
                            feats = np.concatenate((feats.cpu().numpy(), _weight_low*feats_list[idx][None, :]), axis=-1)
                            feat_high_patches.extend(feats)
                    
                if len(high_patches) == 0:
                    mean_feature = feat_high_patches
                else:
                    mean_feature = np.mean(np.array(feat_high_patches), axis=0)

                feats_tree_list.append(list(mean_feature))
            ...
            if len(feats_tree_list) == 0:
                ...
                assert(df.shape[0]==len(low_patches))
                
            print('\n') 
            
              
            
def compute_tree_feats_separate(args, bags_list, embedder_low, embedder_high, save_path=None, fusion='fusion'):
    print ('Using compute_tree_feats_separate function')
    _weight_low = 0.25
    embedder_low.eval()
    embedder_high.eval()
    num_bags = len(bags_list)
    ...
    with torch.no_grad():
        for i in range(0, num_bags): 
            ...
            for idx, low_patch in enumerate(low_patches):
                ...
                feat_high_patches = []                                               # Filmon added this
                ...
                low_feature = _weight_low*feats_list[idx]
                feats_tree_list_low.append(list(low_feature))
                feats_tree_list_high.append(list(mean_feature))
                
                sys.stdout.write('\r Computed: {}/{} -- {}/{}'.format(i+1, num_bags, idx+1, len(low_patches)))
        
            
            if len(feats_tree_list_low) == 0:
                print('No valid patch extracted from: ' + bags_list[i])
            else:
                df_low = pd.DataFrame(feats_tree_list_low)
                os.makedirs(os.path.join(save_path+'_low', bags_list[i].split(os.path.sep)[-2]), exist_ok=True)
                df_low.to_csv(os.path.join(save_path+'_low', bags_list[i].split(os.path.sep)[-2], 
                                       bags_list[i].split(os.path.sep)[-1]+'.csv'), index=False, float_format='%.4f')
                
                df_high = pd.DataFrame(feats_tree_list_high)
                os.makedirs(os.path.join(save_path+'_high', bags_list[i].split(os.path.sep)[-2]), exist_ok=True)
                df_high.to_csv(os.path.join(save_path+'_high', bags_list[i].split(os.path.sep)[-2], 
                                       bags_list[i].split(os.path.sep)[-1]+'.csv'), index=False, float_format='%.4f')
                
                
                assert(df_low.shape[0]==df_high.shape[0])
            print('\n') 
...
def main():
    ...
    parser.add_argument('--num_workers', default=64, type=int, help='Number of threads for datalodaer')
    ...
    parser.add_argument('--holdout_set', default= False, help='Used to create datasets')
    parser.add_argument('--separate_feat', default= False, help='Generated features separately')
    ...
    if args.holdout_set:
        if args.magnification == 'tree' or args.magnification == 'low' or args.magnification == 'high' :
            bags_path = os.path.join('WSI_test_set', args.dataset, 'pyramid', '*', '*')
        else:
            bags_path = os.path.join('WSI_test_set', args.dataset, 'single', '*', '*')
        
        feats_path = os.path.join('datasets_test_set', args.dataset)
        print (bags_path)
        print (feats_path)
    else:
        if args.magnification == 'tree' or args.magnification == 'low' or args.magnification == 'high':
    ...
    if args.magnification == 'tree':
        if args.separate_feat:
            compute_tree_feats_separate(args, bags_list, i_classifier_l, i_classifier_h, feats_path, fusion='separate')
        else:
    ...
    if args.holdout_set:
        n_classes = glob.glob(os.path.join('datasets_test_set', args.dataset, '*'+os.path.sep))
    else:
    ...
    for i, item in enumerate(n_classes):
        ...
        if args.holdout_set:
            bag_df.to_csv(os.path.join('datasets_test_set', args.dataset, item.split(os.path.sep)[2]+'.csv'), index=False)
        else:
    ...
    if args.holdout_set:
        bags_path.to_csv(os.path.join('datasets_test_set', args.dataset, args.dataset+'.csv'), index=False)
    else:
