...
def compute_feats(args, bags_list, i_classifier, save_path=None, whole_slide_path=None):
    ...
    for i in range(0, num_bags):
        ...
        if (
            os.path.isdir(os.path.join(save_path, "simclr_files", file_name))
        ...
        txt_file = open(
            os.path.join(save_path, "simclr_files", file_name, "c_idx.txt"), "w+"
        )
        save_coords(txt_file, csv_file_path)
        # save node features
        output = torch.stack(feats_list, dim=0).cuda()
        torch.save(
            output, os.path.join(save_path, "simclr_files", file_name, "features.pt")
        )
        # save adjacent matrix
        adj_s = adj_matrix(csv_file_path, output)
        torch.save(
            adj_s, os.path.join(save_path, "simclr_files", file_name, "adj_s.pt")
        )
