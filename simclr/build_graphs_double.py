...
def compute_feats(
    ...
    for i in range(0, num_bags):
        ...
        if (
            os.path.isdir(
                os.path.join(save_path, f"{simclr_files_folder}", file_name_files)
            )
            or len(csv_file_path) < 1
        ):
        ...
        txt_file = open(
            os.path.join(
                save_path, f"{simclr_files_folder}", file_name_files, "c_idx.txt"
            ),
            "w+",
        )
        ...
        if holdout_set:
            ...
            features_double = pd.read_csv(
                f"/workspace/data/cv_methods/gt/datasets_test_set/{bcc_folder}/data/{file_name}.csv"
            )
            ...
            if separate_feat == "low":
                ...
                bcc_folder = "bcc"
            features_double = pd.read_csv(
                f"/workspace/data/cv_methods/gt/datasets/{bcc_folder}/data/{file_name}.csv"
            )

        output = torch.tensor(np.array(features_double)).cuda()
        # save_path = '/workspace/data/cv_methods/gt/graphs/'

        torch.save(
            output,
            os.path.join(
                save_path, f"{simclr_files_folder}", file_name_files, "features.pt"
            ),
        )
        ...
        torch.save(
            adj_s,
            os.path.join(
                save_path, f"{simclr_files_folder}", file_name_files, "adj_s.pt"
            ),
        )
        print("\r Computed: {}/{}".format(i + 1, num_bags))

        adj_s = torch.load(
            f"../graphs/{simclr_files_folder}/{file_name}_files/adj_s.pt",
            map_location=lambda storage, loc: storage,
        )

        features_double = torch.load(
            f"../graphs/{simclr_files_folder}/{file_name}_files/features.pt",
            map_location=lambda storage, loc: storage,
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute BCC features from SimCLR embedder"
    )
    parser.add_argument(
        "--num_classes", default=2, type=int, help="Number of output classes"
    )
    parser.add_argument("--num_feats", default=512, type=int, help="Feature size")
    parser.add_argument(
        "--batch_size", default=128, type=int, help="Batch size of dataloader"
    )
    parser.add_argument(
        "--num_workers", default=0, type=int, help="Number of threads for datalodaer"
    )
    parser.add_argument("--dataset", default=None, type=str, help="path to patches")
    parser.add_argument(
        "--backbone", default="resnet18", type=str, help="Embedder backbone"
    )
    parser.add_argument(
        "--magnification",
        default="20x",
    ...
    parser.add_argument(
        "--weights", default=None, type=str, help="path to the pretrained weights"
    )
    parser.add_argument(
        "--output", default=None, type=str, help="path to the output graph folder"
    )
    parser.add_argument(
        "--holdout_set", default=False, help=" Used to read from datasets"
    )
    parser.add_argument(
        "--separate_feat", default=None, type=str, help="Generated features separately"
    )
    ...
    if args.holdout_set:
        ...
        bags_list = glob.glob("/workspace/data/cv_methods/gt/WSI/bcc/pyramid/data/*/")
