...
def compute_feats(args, bags_list, i_classifier, save_path=None, whole_slide_path=None, holdout_set=False, separate_feat=None):
    ...
    for i in range(0, num_bags):
        ...
        if  args.magnification == '20x' and separate_feat == 'low':
            csv_file_path = glob.glob(os.path.join(bags_list[i], '*.jpeg'))
            file_name = bags_list[i].split('/')[-2]
            class_name = bags_list[i].split('/')[-3]
            
        elif  args.magnification == '20x' and separate_feat == 'high':
            csv_file_path = glob.glob(os.path.join(bags_list[i], '*','*.jpeg'))
            file_name = bags_list[i].split('/')[-2]
            class_name = bags_list[i].split('/')[-3]
            
        elif args.magnification == '20x':
            csv_file_path = glob.glob(os.path.join(bags_list[i], '*.jpeg'))
            file_name  = bags_list[i].split('/')[-2]
            class_name = bags_list[i].split('/')[-3]
        ...
        file_name_files = file_name + '_files'
        ...
        if os.path.isdir(os.path.join(save_path, f'{simclr_files_folder}', file_name_files)) or len(csv_file_path) < 1:
            print('already exists')
            continue
        #with torch.no_grad():
        #    for iteration, batch in enumerate(dataloader):
        #        continue                                                                              # Filmon removed this 
        ...
        os.makedirs(os.path.join(save_path, f'{simclr_files_folder}', file_name_files), exist_ok=True)
        
        txt_file = open(os.path.join(save_path, f'{simclr_files_folder}', file_name_files, 'c_idx.txt'), "w+")
        ...
        if holdout_set:               
            ...
            features_double = pd.read_csv(f'/workspace/data/cv_methods/tmi2022/datasets_test_set/{bcc_folder}/data/{file_name}.csv')
            ...
            if separate_feat == 'low':
                ...
                bcc_folder = 'bcc'
            features_double = pd.read_csv(f'/workspace/data/cv_methods/tmi2022/datasets/{bcc_folder}/data/{file_name}.csv')
            
        
        output = torch.tensor(np.array(features_double)).cuda()
        #save_path = '/workspace/data/cv_methods/tmi2022/graphs/'

  
        torch.save(output, os.path.join(save_path, f'{simclr_files_folder}', file_name_files, 'features.pt'))
    
        # save adjacent matrix
        adj_s = adj_matrix(csv_file_path, output)
        torch.save(adj_s, os.path.join(save_path, f'{simclr_files_folder}', file_name_files, 'adj_s.pt'))
        print('\r Computed: {}/{}'.format(i+1, num_bags))
        
        adj_s = torch.load(f'../graphs/{simclr_files_folder}/{file_name}_files/adj_s.pt', 
                           map_location=lambda storage, loc: storage)   
        
        features_double = torch.load(f'../graphs/{simclr_files_folder}/{file_name}_files/features.pt',
                                     map_location=lambda storage, loc: storage)

    
        assert(adj_s.shape[0]==features_double.shape[0])


def main():
    parser = argparse.ArgumentParser(description='Compute BCC features from SimCLR embedder')
    ...
    parser.add_argument('--holdout_set',   default= False, help=' Used to read from datasets')   # Filmon added this
    parser.add_argument('--separate_feat', default= None, type=str, help='Generated features separately') # Filmon added this
    ...
    #bags_list = glob.glob(args.dataset +'*/20.0')
                
    if args.holdout_set:
        bags_list = glob.glob('/workspace/data/cv_methods/tmi2022/WSI_test_set/bcc/pyramid/data/*/')           
    else:
        bags_list = glob.glob('/workspace/data/cv_methods/tmi2022/WSI/bcc/pyramid/data/*/')
            
    
    #bags_list = glob.glob(args.dataset)
    #print(args.dataset + "*/20.0")
    
    compute_feats(args, bags_list, i_classifier, args.output, holdout_set=args.holdout_set, separate_feat=args.separate_feat)
